 ...
 * @return {Object} Returns a query result with a list of ancestor content (result.hits)
 */
export function getAncestors({
	...
	const pathParts = content._path.split('/').slice(includeCurrent ? 1 : 2);
	//log.info(toStr({pathParts}));
	addFilter({
		clause: 'should',
		filter: hasValue(
			'_path',
	...
	const queryRes = queryContent(queryParams);
